...
+ (void)jumpToLocation:(NSString *)target_url viewController:(UIViewController *)vc{
    ...
    if ([target_url isEqualToString:@"com.danlai.ndpz://app/v1/usercoupons/method"]){
        //跳转到用户未过期优惠券列表
...
+(void) jumpToTrade:(NSString *)target_url viewController:(UIViewController *)vc{
    ...
    if ([firstparam isEqualToString:@"trade_id"]){
        ...
        orderDetailVC.urlString = [NSString stringWithFormat:@"%@/rest/v2/trades/%@?device=app&show_sales=1", Root_URL, firstvalue];
