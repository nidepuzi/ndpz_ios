...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.cellImageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"icon_placeholderEmpty"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (image) {
...
- (void)composeWithShareModel:(JMSharePicModel *)model Button:(UIButton *)button {
    if (button.tag == 100) { // 保存图片
        ...
        if (pab == nil) {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"铺子小贴士" message:@"亲爱的铺子用户,现在文案复制成功了哦~可以去朋友圈粘贴回复啦。" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alert show];
...
- (void)alertMessage {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"分享小贴士" message:@"亲爱的铺子用户,现在可以直接分享微信了哦~点击'确定'就可以直接发朋友圈啦,点击'取消'本次不再提示此条信息。" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    alert.tag = 102;
    [alert show];
}
