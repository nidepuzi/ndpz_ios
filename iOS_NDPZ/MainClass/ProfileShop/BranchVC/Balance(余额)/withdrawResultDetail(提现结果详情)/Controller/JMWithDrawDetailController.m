...
- (void)loadStatusData:(NSString *)urlString {
    [JMHTTPManager requestWithType:RequestTypeGET WithURLString:urlString WithParaments:nil WithSuccess:^(id responseObject) {
        ...
        if (results.count == 0) {
            self.moneyLabel.text = @"暂无数据,请稍后查询。 如有疑问请询问你的铺子客服哦~!";
        }else {
...
//    }else {
        ...
        if ([dict[@"budget_type"] boolValue]) {
            ...
            timeLineTitleArr = @[@"出账申请",statusDescStr,@"出账成功"];
        }else {
            self.takeoutMoney.text = @"收入金额(元)";
            budgetType = @"收 入 方 式:";
            timeLineTitleArr = @[@"收入申请",statusDescStr,@"收入成功"];
        }
