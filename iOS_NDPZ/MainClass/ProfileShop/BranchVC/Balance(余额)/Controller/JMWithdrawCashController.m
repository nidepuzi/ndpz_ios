...
- (void)createWtihdrawView {
    ...
    CGFloat budgerCash = 0.00;
    if ([_userBudget isKindOfClass:[NSDictionary class]]) {
        budgerCash = [_userBudget[@"budget_cash"] floatValue];
    }
    ...
    [self.withdrawButton setImage:[UIImage imageNamed:@"cs_yuanquan_nomal"] forState:UIControlStateNormal];
    [self.withdrawButton setImage:[UIImage imageNamed:@"cs_yuanquan_selected"] forState:UIControlStateSelected];
...
//    }
    NSString *allString = @"1.您可以随时提现 \n2.提现金额将以微信红包形式发到您绑定的微信账户 \n3.通常铺子审核日后3个工作日内到账 \n4.审核日是每个月的9号和24号 (9号审核时间范围: 上月24号17:01至本月9号17:00) \n5.如有其它疑问,请联系你的铺子服务号nidepuzi";
    self.descTitleLabel.attributedText = [JMRichTextTool cs_changeLineSapceWithAllString:allString LineSpace:5];
