...
@property (nonatomic, strong) UIButton *lianxikefuBtn;
...
- (void)setUpTopUI {
    ...
    UIButton *lianxikefuBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.bottomView addSubview:lianxikefuBtn];
    [lianxikefuBtn setImage:[UIImage imageNamed:@"cs_lianxikefu_icon"] forState:UIControlStateNormal];
    [lianxikefuBtn setTitle:@"联系客服" forState:UIControlStateNormal];
    lianxikefuBtn.titleLabel.font = CS_UIFontSize(16.);
    [lianxikefuBtn setTitleColor:[UIColor buttonTitleColor] forState:UIControlStateNormal];
    lianxikefuBtn.tag = 102;
    [lianxikefuBtn addTarget:self action:@selector(outDateClick:) forControlEvents:UIControlEventTouchUpInside];
    self.lianxikefuBtn = lianxikefuBtn;
    ...
    [lianxikefuBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.top.equalTo(weakSelf.bottomView);
        make.width.mas_equalTo(SCREENWIDTH);
        make.height.mas_equalTo(@60);
    }];
    // === 显示红包 === //
